...
import { getAllowedBasicOrbTypes, getBasicOrbCategoryGroups, normalizeAbilityType, applyOrbEffectsToStats, formatAbilityLabel, formatNumberWithCommas} from './orbRules.js';
...
function updateMutantStatsUI(currentStats, selectedOrbs, context) {
  // 1. Obtener los stats calculados (números limpios)
  const calculatedStats = applyOrbEffectsToStats(currentStats, selectedOrbs, context);

  // 2. Formatear y renderizar en el DOM (Aplica con o sin orbes)
  
  // Vida
  const lifeElement = document.getElementById('stat-life');
  if (lifeElement) {
    lifeElement.textContent = formatNumberWithCommas(calculatedStats.lifeF);
  }

  // Ataque 1
  const atk1Element = document.getElementById('stat-atk1');
  if (atk1Element) {
    atk1Element.textContent = formatNumberWithCommas(calculatedStats.atk1F);
  }

  // Ataque 2
  const atk2Element = document.getElementById('stat-atk2');
  if (atk2Element) {
    atk2Element.textContent = formatNumberWithCommas(calculatedStats.atk2F);
  }

  // Habilidad Ataque 1
  const ability1Element = document.getElementById('stat-ability1');
  if (ability1Element) {
    ability1Element.textContent = formatNumberWithCommas(calculatedStats.atk1AbilityF);
  }

  // Habilidad Ataque 2
  const ability2Element = document.getElementById('stat-ability2');
  if (ability2Element) {
    ability2Element.textContent = formatNumberWithCommas(calculatedStats.atk2AbilityF);
  }
}
